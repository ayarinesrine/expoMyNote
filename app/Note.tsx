...
import { SafeAreaProvider } from "react-native-safe-area-context";
import AsyncStorage from "@react-native-async-storage/async-storage";
import UpdateScreen from "./Update"; // Import the update screen component
...
const NoteScreen: React.FC = () => {
  ...
  const goToUpdateScreen = (noteId: string) => {
    navigation.navigate("Update", { noteId });
  };
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      <SafeAreaProvider>
        <Appbar.Header>
          ...
          <TouchableOpacity
            style={styles.updateButton}
            onPress={() => goToUpdateScreen(note.id)}
          >
            <MaterialCommunityIcons
              name="border-color"
              size={22}
